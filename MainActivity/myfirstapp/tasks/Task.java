...
public class Task {
    ...
    public boolean hasEnergyLevel() {
        return energyLevel != null && !energyLevel.isEmpty();
    }

    public String getEnergyLevelLabel() {
    ...
    public int getEnergyLevelColor() {
        if (energyLevel == null) return 0xFF64748B;
        switch (energyLevel) {
            case "deep_work":  return 0xFFF59E0B; // amber
            case "light":      return 0xFF10B981; // green
            case "low_energy": return 0xFF64748B; // slate
    ...
    public boolean hasLocationReminder() {
        return locationReminderName != null && !locationReminderName.isEmpty();
    }
    ...
    public static Task fromJson(JSONObject json) {
        ...
        try {
            ...
            // New fields (Prompt 1 extension)
            String energy = json.optString("energyLevel", "");
            if (energy.isEmpty()) energy = json.optString("energy_level", "");
            task.energyLevel = energy.isEmpty() ? null : energy;
            String locName = json.optString("locationReminderName", "");
            if (locName.isEmpty()) locName = json.optString("location_reminder_name", "");
